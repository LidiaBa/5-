...
for (let num = 1; num < 4; num++) { //создание 3 студентов изначально
data.create({
name: "Студент " + num
...
data.update(st); //записывает данные о студенте
}
this.render(); //данные на странице выводились правильно
util.id("fieldset_creation").style.display = "none"; //после кнопки окно убиралось
}

this.remove = () => {
data.delete(activeStudent); //удаляет студента
this.render(); //обновляет форму, чтобы было видно удаление
util.id("fieldset_deletion").style.display = "none"; //скрывает форму "вы действительно хотите удалить студента"
}
...
const addListener = ()=>{
util.id("to_view_form_add").addEventListener("click", add); //кнопка "добавить"
